...
if(!$gperm->group_perm(1)){
	redirect_header($mydirurl, 0, _NOPERM);
	exit();
}
...
if( !$BTopic->topicExists() ){
	redirect_header($mydirurl, 3, _MD_NO_TOPICS);
	exit;
}
...
$int_arr = array('topicid','type','ihome','topicimg','published','expired','approve','movetotop');
$bai_arr = array('html','smiley','br','xcode','autodate','autoexpdate','notifypub');
...
if( $op == 'post' ){
	...
	$auto['sec']   = isset( $_POST['auto']['sec'] )   ? intval( $_POST['auto']['sec'] )   : date('i');
	...
	$autoexp['sec']   = isset( $_POST['autoexp']['sec'] )   ? intval( $_POST['autoexp']['sec'] )   : date('i');
	...
	// edited post
		...
		// 掲載予定日設定のルーチン
		if( $story->getVar('published') < $time && $story->getVar('approve') == 1 ){
			$story->setVar('published', $time);
		} elseif ( $story->getVar('movetotop') == 1 ){
			$story->setVar('published', $time);
		} elseif ( $story->getVar('autodate') == 1 ){
			$pubdate = mktime( $auto['hour'], $auto['min'], $autoexp['sec'], $auto['month'], $auto['day'], $auto['year'] );
	...
	if( $is_new ){
		...
		if ( $return == 1 ){
			...
			redirect_header($mydirurl, 3, _MD_DBPUDATED);
		}
		exit;
